...
def parse_product(url):
    ...
    # Price4 normally exposes the retailer list as a real HTML table.
    for tr in soup.find_all("tr"):
        ...
        if not store or store.lower() in {"store", "total", "visit store"} or len(store) > 80:
            continue
        prices = []
        for text in texts[1:]:
    ...
    # Fallback: some Price4 responses expose the comparison rows as divs.
    # Their extracted text is typically: retailer / £price / Visit Store.
    if not retailers:
        lines = [re.sub(r"\s+", " ", x).strip() for x in soup.get_text("\n").splitlines()]
        start = 0
        for i, line in enumerate(lines):
            if "Store" in line and "Total" in line:
                start = i + 1
                break

        for i in range(start, len(lines)):
            line = lines[i]
            prices = money_values(line)
            if not prices or "Visit Store" in line:
                continue

            store = None
            for j in range(i - 1, max(start - 1, i - 5), -1):
                candidate = clean_store_name(lines[j])
                if not candidate or candidate.lower() in {"store", "total", "visit store"}:
                    continue
                if "Image:" in lines[j]:
                    store = candidate
                    break
                if not re.search(r"£\s*\d", candidate) and len(candidate) <= 80:
                    store = candidate
                    break

            if store and store.lower() not in {"best price", "about"}:
                retailers[store] = round(prices[0], 2)
